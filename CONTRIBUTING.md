# Contributing to Electron Installer: Snap

Electron Installer: Snap is a community-driven project. As such, we welcome and encourage all sorts
of contributions. They include, but are not limited to:

- Constructive feedback
- [Questions about usage](https://github.com/electron-userland/electron-installer-snap/blob/master/SUPPORT.md)
...
calling `electron-installer-snap`. This will print debug information from the specified modules. The
...
* `electron-installer-snap:*` (always use this one before filing an issue)
...
**If you are using `npm run` to execute `electron-installer-snap`, run the `electron-installer-snap`
command without using `npm run` and make a note of the output, because `npm run` does not print out
error messages when a script errors.**

## Contribution suggestions

We use the label [`help wanted`](https://github.com/electron-userland/electron-installer-snap/issues?q=is%3Aopen+is%3Aissue+label%3A%22help+wanted%22) in the issue tracker to denote fairly-well-scoped-out bugs or feature requests that the community can pick up and work on. If any of those labeled issues do not have enough information, please feel free to ask constructive questions. (This applies to any open issue.)
